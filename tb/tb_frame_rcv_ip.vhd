...
architecture sim of tb_frame_rcv_ip is
    ...
    signal synthetic_eop_count     : integer := 0;
...
begin
    ...
    begin
        if rising_edge(i_clk) then
            if i_rst = '1' then
                ...
                elsif aso_hit_type0_endofpacket = '1' then
                    synthetic_eop_count <= synthetic_eop_count + 1;
                end if;
    ...
    stim : process
        ...
        variable base_synth_eops    : integer;
        variable base_headerinfo    : integer;
        variable ready_seen         : boolean;
        variable synth_marker_seen  : boolean;
    ...
    begin
        ...
        base_synth_eops := synthetic_eop_count;
        send_ctrl_until_ready(i_clk, asi_ctrl_data, asi_ctrl_valid, asi_ctrl_ready, CTRL_TERMINATE_CONST);
        synth_marker_seen := false;
        for wait_idx in 0 to 5 loop
            wait until rising_edge(i_clk);
            if aso_hit_type0_endofpacket = '1' and aso_hit_type0_valid = '0' then
                synth_marker_seen := true;
            end if;
        end loop;
        assert synth_marker_seen
            report "Idle TERMINATING should emit exactly one synthetic hit_type0 EOP marker"
            severity failure;
        assert synthetic_eop_count = base_synth_eops + 1
            report "Synthetic EOP monitor count did not advance as expected"
            severity failure;
        ...
        assert hit_eop_count = base_hit_eops + 1
            ...
            severity failure;
        assert synthetic_eop_count = base_synth_eops
            report "Active frame drain should not create an extra synthetic EOP"
            severity failure;
